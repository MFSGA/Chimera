...
- Each top-level spec starts and ends with the harness closing non-legacy windows and restoring the legacy entry URL. Specs must not depend on a main/editor window or route left behind by an earlier spec; persistent application/config state must still be restored explicitly by the owning test.
