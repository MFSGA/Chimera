...
- Verifies that the packaged frontend loads from `tauri.localhost`, React renders into `#root`, and primary sidebar navigation works.
...
Each test run creates a UUID-named directory under `tauri-e2e/.tmp/runtime` and passes its `config` and `data` subdirectories through E2E-only environment variables. Dedicated runtime-isolation tests run before the desktop test to verify consecutive runs cannot reuse prior state. This prevents the test process from opening the developer instance's configuration or `storage.db` while leaving normal builds unchanged.

The E2E feature also skips service installation, core process startup, and system-proxy initialization. This keeps desktop UI tests isolated from machine-wide networking state.
