...
- Each top-level spec starts and ends with the harness closing temporary non-app windows, focusing `legacy`, and restoring the legacy entry URL. The singleton `main` window is preserved when present; main specs must explicitly enter their target route instead of depending on a route left by an earlier spec. Persistent application/config state must still be restored explicitly by the owning test.
