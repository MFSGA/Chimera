...
async function runSeedPhase(): Promise<void> {
  ...
  const clash = typedPath('clash-config.yaml');
  const session = typedPath('session-state.yaml');
  const migrationState = typedPath('migration-state.yaml');
  ...
  assert.equal(fs.existsSync(migrationState), true);
  assert.equal(persistedBoolean(application, 'enable_auto_launch'), true);
  assert.equal(persistedBoolean(application, 'enable_system_proxy'), false);
  assert.equal(persistedBoolean(clash, 'ipv6'), true);
  assert.deepEqual(persistedMainWindowState(session), {
    width: 820,
    height: 620,
    x: 20,
    y: 20,
...
describeUpgrade('Windows v0.22.3 to v0.23.0 typed-state upgrade', () => {
