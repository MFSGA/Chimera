...
mod tests {
    ...
    #[test]
    fn legacy_uncertainty_is_coalesced() {
        let primary = ClientError::Custom("primary".into());
        let partial = PartialCommit::new(
            &primary,
            vec![LegacyVergeDomain::Application],
            Vec::new(),
            vec![CompensationFailure::LegacyStateUncertain {
                message: "first".into(),
            }],
        )
        .with_legacy_state_uncertain("second".into());

        assert_eq!(partial.failed_compensations.len(), 1);
        assert_eq!(
            partial.failed_compensations,
            vec![CompensationFailure::LegacyStateUncertain {
