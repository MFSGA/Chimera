...
pub(crate) struct NetworkProbeRequest {
    pub url: String,
...
pub(crate) struct NetworkProbeResult {
    pub status: u16,
    pub expected_status: Option<u16>,
...
pub(crate) enum NetworkProbeError {
    #[error("network_probe_invalid_request")]
    InvalidRequest,
    #[error("network_probe_invalid_target")]
    InvalidTarget,
    #[error("network_probe_target_blocked")]
    TargetBlocked,
    #[error("network_probe_resolution_failed")]
    ResolutionFailed,
    #[error("network_probe_timed_out")]
    TimedOut,
    #[error("network_probe_execution_failed")]
    ExecutionFailed,
}
...
) -> Result<NetworkProbeResult, NetworkProbeError> {
    execute_with_policy(request, false).await
}

/// Probe a URL explicitly supplied by the user.
///
/// Unlike the agent tool, this path may inspect private or local addresses so
/// that user-owned LAN subscription endpoints remain usable. The URL is still
/// restricted to HTTP(S), credentials are rejected, and the request bypasses
/// configured proxies.
pub(crate) async fn execute_user_target(
    request: NetworkProbeRequest,
) -> Result<NetworkProbeResult, NetworkProbeError> {
    execute_with_policy(request, true).await
}
...
async fn execute_with_policy(
    ...
    let target = validate_probe_request(request, allow_private_targets)?;
    let deadline = Instant::now() + target.timeout;
    let (domain, addresses) =
        resolve_probe_target(&target.url, target.timeout, allow_private_targets).await?;
    let remaining = deadline
        .checked_duration_since(Instant::now())
        .ok_or(NetworkProbeError::TimedOut)?;
    ...
    let mut client = reqwest::Client::builder()
        ...
        .user_agent("Chimera-Network-Probe");
    ...
    let client = client
        .build()
        .map_err(|_| NetworkProbeError::ExecutionFailed)?;
    let started = Instant::now();
    let response = client.get(target.url).send().await.map_err(|error| {
        if error.is_timeout() {
            NetworkProbeError::TimedOut
        } else {
            NetworkProbeError::ExecutionFailed
        }
    })?;
...
fn validate_probe_request(
    request: NetworkProbeRequest,
    allow_private_targets: bool,
) -> Result<ProbeTarget, NetworkProbeError> {
    if request.url.len() > MAX_NETWORK_PROBE_URL_BYTES {
        return Err(NetworkProbeError::InvalidRequest);
    }
    let url = Url::parse(&request.url).map_err(|_| NetworkProbeError::InvalidTarget)?;
    ...
    if !allow_private_targets {
        match url.host().ok_or(NetworkProbeError::InvalidTarget)? {
            Host::Domain(domain) if is_blocked_hostname(domain) => {
                return Err(NetworkProbeError::TargetBlocked);
            }
...
async fn resolve_probe_target(
    ...
    allow_private_targets: bool,
) -> Result<(Option<String>, Vec<SocketAddr>), NetworkProbeError> {
    let port = url
        .port_or_known_default()
        .ok_or(NetworkProbeError::InvalidTarget)?;
    match url.host().ok_or(NetworkProbeError::InvalidTarget)? {
        ...
        Host::Domain(domain) => {
            ...
            .await
            .map_err(|_| NetworkProbeError::ResolutionFailed)?
            .map_err(|_| NetworkProbeError::ResolutionFailed)?;
            let addresses = collect_safe_addresses(resolved, allow_private_targets)?;
...
fn collect_safe_addresses(
    ...
    for address in resolved {
        if !allow_private_targets && is_blocked_ip(address.ip()) {
            return Err(NetworkProbeError::TargetBlocked);
        }
    ...
    if addresses.is_empty() {
        return Err(NetworkProbeError::ResolutionFailed);
    }
    Ok(addresses)
}
...
mod tests {
    ...
    use super::{
        NetworkProbeError, NetworkProbeRequest, collect_safe_addresses, is_blocked_hostname,
        is_blocked_ip, validate_probe_request,
    };
    ...
    fn accepts_public_http_targets_with_bounded_timeout() {
        let target = validate_probe_request(
            NetworkProbeRequest {
    ...
    fn rejects_credentials_invalid_status_and_out_of_range_timeout() {
        for request in [
            ...
            assert!(validate_probe_request(request, false).is_err());
        }
    }
    ...
    fn rejects_resolution_if_any_address_is_not_public() {
        ...
        assert!(matches!(result, Err(NetworkProbeError::TargetBlocked)));
    }

    #[test]
    fn user_supplied_private_targets_are_allowed_but_agent_targets_are_blocked() {
        let request = NetworkProbeRequest {
            url: "http://127.0.0.1:8080/subscription.yaml".to_owned(),
            expected_status: None,
            timeout_ms: None,
        };

        assert_eq!(
            validate_probe_request(request.clone(), false).unwrap_err(),
            NetworkProbeError::TargetBlocked
        );
        assert!(validate_probe_request(request, true).is_ok());

        let private_addresses = [SocketAddr::from(([127, 0, 0, 1], 8080))];
        assert!(collect_safe_addresses(private_addresses, true).is_ok());
    }
}
