...
Public Bridge responses use request IDs and structured errors. Tool calls are subject to bounded timeouts, an in-flight concurrency limit, and per-window rate limiting. Authentication, tool lookup, body-size enforcement, and request-schema validation happen before an execution permit is acquired, so rejected traffic cannot consume a tool slot or access application state. Tool execution remains inside the HTTP request future: when a client disconnects before completion, the execution future is cancelled and its semaphore permit is released by RAII, allowing later requests to proceed. Invalid authentication, malformed payloads, unknown tools, rejected arguments, timeouts, overload, and internal failures are returned as stable error codes rather than raw internal errors. Error codes and messages are compile-time static strings; HTTP canary tests verify that rejected credentials, unknown tool names, target URLs, query parameters, and request bodies are never reflected in an error response. Additional integration tests verify status codes and request-ID correlation, prove that excess concurrent requests never enter the executor, confirm that unauthenticated traffic does not consume the authenticated request budget, and cover disconnect cancellation with permit reuse.
...
| `tun.status`       | Return desired and generated TUN state and consistency.                                              |
