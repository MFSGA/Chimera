import { openThat, useProfile } from '@chimera/interface';
...
const FieldButton = ({
  ...
  return (
    <Button
      ...
      disabled={disabled}
      onClick={() => void blockTask.execute()}
...
const ItemButton = ({
  ...
  const isChimeraControlField = ['default', 'handle'].includes(
    children as string,
  );
              ...
              <div className="grid grid-cols-2 gap-2">
                {Object.entries(items).map(([item, data]) => (
                  <FieldButton
                    key={item}
                    data={data}
                    disabled={isChimeraControlField}
                    label={item}
                  />
                ))}
              </div>
  ...
  return (
    <div className="grid grid-cols-2 gap-2 lg:grid-cols-4">
      {Object.entries(CLASH_FIELD).map(([key, value]) => (
        <ItemButton key={key} items={filteredField(value)}>
          {key}
        </ItemButton>
      ))}
    </div>
