import { useIPSB, useSetting } from '@chimera/interface';
...
const countryCodeEmoji = (countryCode: string) => {
  if (!/^[a-z]{2}$/i.test(countryCode)) {
    return String.fromCodePoint(0x1f1fa, 0x1f1f3);
  }
...
const EmojiCountry = ({ countryCode }: { countryCode: string }) => {
    ...
    >
      <Paper className="relative flex !h-full gap-4 !rounded-3xl px-4 py-3 select-text">
        {data ? (
          <>
            {data.country_code && (
              <EmojiCountry countryCode={data.country_code} />
            )}
