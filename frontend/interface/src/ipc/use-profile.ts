...
export const useProfile = (options?: { without_helper_fn?: boolean }) => {
  ...
  const normalizeProfile = (item: Profile_Serialize): NormalizedProfile => {
    return (item.remote ?? item.local) as NormalizedProfile;
  };
  ...
  const query = useQuery({
    ...
    queryFn: async (): Promise<ProfilesQueryResult> => {
      ...
      const items = result.items.map((item) => normalizeProfile(item));
